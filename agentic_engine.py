...
def run_agentic_cycle(final_report, execute_enabled=False):
    run_id = datetime.now(timezone.utc).strftime("agent-%Y%m%d-%H%M%S")
    memory_state = _load_memory_state()
    feedback = _memory_feedback(memory_state)

    previous_run = (memory_state.get("runs") or [])[-1] if memory_state.get("runs") else None
    previous_facts = previous_run.get("observed_facts", {}) if previous_run else {}
    ...
    plan = run_agentic_planner(final_report, memory_feedback=feedback)
    outcome_tracking = _evaluate_action_outcomes(previous_executed_run, plan.get("observed_facts", {}))
    escalation_actions = _derive_escalation_actions(outcome_tracking)
    if escalation_actions:
        combined = plan.get("action_queue", []) + escalation_actions
        plan["action_queue"] = _sort_actions_by_priority(combined)
        plan["escalation_actions"] = escalation_actions

    trend = _trend_analysis(
        trend_previous_facts,
        plan.get("observed_facts", {}),
        comparison_basis=trend_comparison_basis,
    )
    executed_actions = execute_actions(
        plan.get("action_queue", []),
        run_id=run_id,
    ...
    run_record = {
        ...
        "timestamp_utc": datetime.now(timezone.utc).isoformat(),
        "execute_enabled": execute_enabled,
        "observed_facts": plan.get("observed_facts", {}),
        "trend": trend,
        "action_ids": [item.get("action_id") for item in plan.get("action_queue", [])],
    ...
    return {
        "enabled": True,
        "mode": "observe-think-decide-act-track",
        "run_id": run_id,
        "plan": plan,
        ...
        "tracking": {
            "actions_total": len(plan.get("action_queue", [])),
        ...
        "memory": {
            ...
            "last_5_status_totals": feedback.get("status_totals_last_5", {}),
            "recurring_failed_action_ids": feedback.get("recurring_failed_action_ids", []),
            "trend": trend,
        },
        "learning": {
